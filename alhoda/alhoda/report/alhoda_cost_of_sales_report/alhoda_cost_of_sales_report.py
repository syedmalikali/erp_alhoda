...
class GrossProfitGenerator(object):
	...
	def get_returned_invoice_items(self):
		returned_invoices = frappe.db.sql(
			"""
			select
				si.name, si_item.item_code, si.custom_invoice_type,si_item.stock_qty as qty, si_item.base_net_amount as base_amount, si.return_against
			from
	...
	def get_buying_amount(self, row, item_code):
		...
		if item_code in self.non_stock_items and (row.project or row.cost_center):
			...
			# Modified By Malik for Non Stock Item
	...
	def load_invoice_items(self):
		...
		if self.filters.custom_invoice_type:
			conditions += " and `tabSales Invoice`.custom_invoice_type = %(custom_invoice_type)s"
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				`tabSales Invoice`.custom_invoice_type,
			...
			order by
				`tabSales Invoice`.posting_date , `tabSales Invoice`.posting_time """.format(
